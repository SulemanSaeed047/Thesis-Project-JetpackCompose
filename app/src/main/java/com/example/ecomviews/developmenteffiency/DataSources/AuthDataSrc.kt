...
import com.example.ecomviews.developmenteffiency.API.UsersAuthApiService
...
class AuthDataSrc() {
    ...
    suspend fun getAllUsers(): List<SignUpUser>? {
        return suspendCancellableCoroutine { continuation ->
            val retrofit = RetrofitHelper.getInstance() // Your existing Retrofit helper
            val service = retrofit.create(UsersAuthApiService::class.java)
            ...
            continuation.invokeOnCancellation {
                call.cancel() // Cancel the call if coroutine is cancelled
            }
        }
    }
